...
const
    ...
    AlphaDepotOptions = [
        AlphaDepotOption.LIST,
        AlphaDepotOption.ONELINE,
        AlphaDepotOption.STATUS,
        AlphaDepotOption.ADD,
        AlphaDepotOption.REMOVE,
        AlphaDepotOption.FORCE
    ];
...
var PluginAction = new Lang.Class({
    ...
    isStatus() {
        return AlphaDepotOption.STATUS.match(this.option.name);
    }
});

var AlphaDepotCli = new Lang.Class({
    Name: "AlphaDepotCli",

    _init(alphadepot) {
        AlphaDepotOptions.forEach(option => option.registerIn(alphadepot.application));

        this.alphadepot = alphadepot;

        if (typeof alphadepot.application.set_option_context_summary !== "undefined") {
            alphadepot.application.set_option_context_parameter_string("app.js");
            alphadepot.application.set_option_context_description("");
            alphadepot.application.set_option_context_summary(
                "  alphadepot --list [--oneline]\n" +
                "  alphadepot [--force] [--status <plugin>]... [--remove <plugin>]... [--add <plugin>]..."
            );
        }

        this.alphadepot.application.connect("handle_local_options", Lang.bind(this, this._onOptions));
    },
    ...
    _onOptions(application, options) {
        ...
        this.pluginRepository = new PluginRepository(this.alphadepot);

        const loop = GLib.MainLoop.new(null, false);
        let promiseOfReports = (AlphaDepotOption.LIST.in(options)) ?
    ...
    _isCliOptions(options) {
        return AlphaDepotOptions.some(option => option.in(options));
    },

    _promiseOfCategoryReports(options) {
        const oneline = AlphaDepotOption.ONELINE.in(options);
    ...
    _promiseOfActionReports: function (options) {
        const forced = AlphaDepotOption.FORCE.in(options);
        return AlphaDepotOptions
    ...
    _isAlreadyApplied(pluginAction, pluginStatus) {
        ...
        case AlphaDepotOption.ADD.name:
            return pluginStatus.code === 0;
        case AlphaDepotOption.REMOVE.name:
            return pluginStatus.code !== 0;
        default:
            return false;
        }
